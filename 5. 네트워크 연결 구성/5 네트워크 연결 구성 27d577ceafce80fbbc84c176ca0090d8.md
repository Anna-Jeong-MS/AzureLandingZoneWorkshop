...
- `HubVNet/AGWSubnet`의 CIDR (10.0.0.0/24)
- `SpokeVNet/AppSubnet`의 CIDR (10.1.0.0/24)
- 방화벽의 개인 IP (10.0.1.4)
...
8. `연결` 버튼을 클릭합니다.
9. 서브넷 연결에서 가상 네트워크를 `HubVNet`, 서브넷을 `AGWSubnet`으로 선택하고 `확인` 버튼을 클릭합니다.
...
7. 왼쪽 메뉴에서 `설정 > 서브넷`을 선택합니다.
8. `연결` 버튼을 클릭합니다.
9. 서브넷 연결에서 가상 네트워크를 `SpokeVNet`, 서브넷을 `AppSubnet`으로 선택하고 확인 버튼을 클릭합니다.